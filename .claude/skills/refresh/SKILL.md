...
Primary source — `developers.openai.com/codex/` (preferred; check this first). **Domain history:** the tree moved to learn.chatgpt.com in mid-2026 (found 2026-07-11), then back to developers.openai.com with a reorganized layout (found 2026-07-25). Both hosts serve byte-identical content (verified by md5 on 2026-07-25), but the machine index declares `developers.openai.com/codex/*` canonical — cite those URLs in the matrix. Reorg casualties: `developer-commands` split into `cli/slash-commands` + `cli/reference` (both slugs currently serve the same merged "Developer commands" page); marketplace semantics moved out of the Codex tree entirely (see Plugins below); `plan_mode_reasoning_effort`, `agents.max_depth`, `spawn_agents_on_csv`, and the `-i/--image` flag dropped out of the docs. **Two of those four have since returned** — `plan_mode_reasoning_effort` is back in the config reference, and `codex -i` / `--image` (comma-separated paths or a repeated flag) is documented again on `codex/image-inputs` as of 2026-09-07 — so treat "dropped out of the docs" as a snapshot, not a deletion, and re-check before asserting a flag is gone.
...
- Repo: `https://github.com/openai/codex` — README and `/CHANGELOG.md`. **Caveat (2026-09-07):** `gh api repos/openai/codex/contents/CHANGELOG.md` returns a one-line stub, not a changelog. Codex is the one tier-1 tool with no usable CHANGELOG over the API — take version signals from the Releases API below and feature signals from the docs site. (Claude Code's and Antigravity's CHANGELOGs both fetch normally.)
...
- Docs index: `https://antigravity.google/llms.txt` — has a dedicated "Antigravity CLI" section (plus "Antigravity 2.0", SDK, and IDE sections). **The index lags the site — do not treat it as complete.** On 2026-09-07 both `/docs/teamwork` and `/docs/boost` served real content while appearing nowhere in `llms.txt`; they were found only by following in-page links out of `/docs/subagents`. When a page you expect is missing from the index, try the URL anyway (404s are real here) and follow "Learn more in the X documentation" links out of related pages.
...
- Key CLI pages: `cli/overview`, `cli/using`, `cli/features` (/rewind, /tasks, subagents framework), `cli/modes` (execution modes incl. plan), `cli/conversations` (/fork), `cli/subagents` (**new since 2026-07-11** — "Background Tasks & Subagents": /tasks panel, Alt+J teleport, Ctrl+K fast-approve), `cli/plugins` (also covers CLI skills), `cli/sandbox`, `cli/permissions`, `cli/settings`, `cli/reference` (slash-command table + settings.json keys), `cli/gcli-migration`, `cli/commands/agents` (and other `cli/commands/*` pages: codesearch, credits, diff, permissions, resume, statusline, title, usage, **voice** — new 2026-09-07). Also `cli/headless`, which since 2026-09-07 is the canonical source for **model selection**: `agy models`, `--model`, `--effort`, `--agent`, `--print-timeout`, and the `--input-format stream-json` driver loop, all previously CHANGELOG-only. Newer CLI pages not in the list above: `cli/projects`, `cli/artifacts`, `cli/statusline`, `cli/title`, `cli/credits`, `cli/vim-editor-mode`, `cli/best-practices`, `cli/troubleshooting`.
- Key platform pages (shared docs that cover the CLI — confirm CLI applicability before citing, e.g. the hooks doc's `~/.gemini/antigravity-cli` data-dir note): `/docs/hooks` (events + built-in tool catalog incl. `search_web`, `schedule`, `manage_task`), `/docs/subagents` (invoke*subagent workspace options incl. `branch` git worktrees, custom-agent `.md` frontmatter, 10-level nesting cap, inter-agent messaging, /teamwork-preview), `/docs/mcp`, `/docs/skills`, `/docs/permissions`, `/docs/rules-workflows`, `/docs/plugins`, `/docs/teamwork` (**not in llms.txt** — the authoritative `/teamwork-preview` page: roles, integrity modes, project-plan/progress artifacts, paid-plan availability), `/docs/boost` (**not in llms.txt** — `/boost` three-tier reasoning pipeline), `/docs/sidecars` (supervised background processes + the `agentapi` CLI: `new-conversation`, `send-message <conversation_id>` — the Inbound-event-channels row's source; note it carries **no CLI marker** and writes to `~/.gemini/antigravity/sidecar_data/`, so CLI applicability is unconfirmed), `/docs/remote-control` (browser-driven access to your own sessions plus an `agy-daemon` headless installer — human-to-session, \_not* agent-to-agent, so it does not belong on the cross-session row). Since 2026-09-07 there is also a whole **"Antigravity for IDEs"** section (`/docs/ide/*`) — that is Google's own downloadable editor, **not** a VS Code/JetBrains integration, so it does not move the IDE-integration row off 🟡.
...
- **ALWAYS re-verify negative claims, not just the symbols.** Every "not on X", "only Y", "requires Z or later", and "no documented W" in a hover or deep-dive is a claim with an expiry date, and it decays in the one direction that misleads a reader. A stale ✅ is usually still ✅; a stale "not supported on native Windows" tells someone their platform is out when it shipped three versions ago. Grep your own hover text for exclusion words — `not`, `only`, `no`, `unavailable`, `requires` — and check each one against the current page, even when the cell's symbol obviously isn't moving. **Rationale:** the 2026-09-07 run found the cross-session-messaging cell asserting two exclusions the vendor had retired (native Windows in v2.1.234, and all four non-Anthropic providers for same-machine delivery in v2.1.248) and a third on Antigravity's model cell ("Gemini models underneath", while `agy models` lists `claude-sonnet-4-6`). All three were correctly sourced when written; none of the three cells changed symbol, so a symbol-first diff would have shipped every one of them again. This differs from the read-the-whole-page rule above: that one catches sourcing from the wrong section, this one catches correct sourcing that has since expired.
