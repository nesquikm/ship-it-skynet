...
| Hooks                        | [✅][cc-hooks]                                                                       | [🟡 broad lifecycle, still bypassable][cx-hooks]   | [✅][gm-hooks]                                       |
...
>
> No cells changed status on 2026-06-10, but the Codex CLI hooks story moved underneath its 🟡: hooks are now **enabled by default** (the `codex_hooks` feature flag survives only as a deprecated alias), the event surface grew from six to ten (`SubagentStart`, `SubagentStop`, `PreCompact`, `PostCompact` joined), and `PreToolUse` gained `updatedInput` tool-input rewriting — previously a Gemini-only trick. Codex also picked up a `/fork` slash command, project-scoped MCP config (`.codex/config.toml`, trusted projects only), a `codex mcp` management CLI, and a per-agent `mcp_servers` field in custom-agent TOML. Claude Code's hook catalog grew to 30 events (`Setup`, `UserPromptExpansion`, `MessageDisplay`, `PostToolBatch`). The Gemini CLI column re-verified clean across all 19 canonical pages — geminicli.com now carries a site-wide banner confirming the 2026-06-18 consumer sunset, and v0.46.0 shipped the morning of this refresh regardless.
