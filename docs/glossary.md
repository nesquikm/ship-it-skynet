...
| Vendor          | Term                    | Notes                                                                                                                                                                                                                                                                                          |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Claude Code     | cross-session messaging | `ListAgents` + `SendMessage` tools, `/list-agents` (alias `/peers`); sessions addressed by name (`--name` / `/rename`); per-session Unix inbox socket on-machine (named pipe on Windows), cold-callable across machines via Remote Control since v2.1.225; v2.1.224+, native Windows v2.1.234+ |
...
### Inbound event channel _(neutral)_

A path for something outside the CLI — CI, a chat platform, a webhook, a timer — to deliver a message into a session that is **already running**, rather than starting a new one. The inverse of cross-session messaging: there, agents talk to each other; here, the world talks to an agent.

| Vendor          | Term             | Notes                                                                                                                                                                                                                                                                                      |
| --------------- | ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Claude Code     | channel          | An MCP server that pushes events into the open session, optionally two-way so Claude replies back out through it; installed as a plugin with your own credentials (Telegram / Discord / iMessage, each needing Bun). Research preview; needs claude.ai or Console auth                     |
| Codex CLI       | ❌ no equivalent | `codex debug app-server send-message-v2` is a protocol test client that starts its own thread, under a command tree documented as development-and-debugging only. Scheduled and app-event work runs in ChatGPT web/desktop; the Slack integration dispatches cloud tasks                   |
| Antigravity CLI | sidecar          | A supervised background process (`sidecar.json`, `restart_policy`, `schedule` builtin) that reaches the agent through the bundled `agentapi` CLI — `new-conversation <prompt>` or `send-message <conversation_id> <prompt>`. Off unless enabled per-sidecar; CLI applicability unconfirmed |

Note the shape mismatch this row exists to flag: Claude Code names the concept and ships an abstraction for it, while Antigravity never uses the word "channel" — you assemble the same capability from a process supervisor plus a CLI. Searching Antigravity's docs for "channel" finds nothing; searching for what you actually want finds `sidecars`.
