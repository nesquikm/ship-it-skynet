...
Full event list (31 as of 2026-08-19): `SessionStart`, `Setup`, `UserPromptSubmit`, `UserPromptExpansion`, `PreToolUse`, `PermissionRequest`, `PermissionDenied`, `PostToolUse`, `PostToolUseFailure`, `PostToolBatch`, `Notification`, `MessageDisplay`, `SubagentStart`, `SubagentStop`, `TaskCreated`, `TaskCompleted`, `Stop`, `StopFailure`, `TeammateIdle`, `InstructionsLoaded`, `ConfigChange`, `CwdChanged`, `DirectoryAdded`, `FileChanged`, `WorktreeCreate`, `WorktreeRemove`, `PreCompact`, `PostCompact`, `Elicitation`, `ElicitationResult`, `SessionEnd`. Newest addition (since the 2026-07-25 refresh): `DirectoryAdded`, fired when a working directory joins the session mid-run via `/add-dir` or the SDK `register_repo_root` control request — observe-only, with no decision control. Earlier additions: `Setup` (fires only with `--init-only`, or `--init`/`--maintenance` in `-p` mode), `UserPromptExpansion` (when a typed command expands into a prompt — can block the expansion), `MessageDisplay` (display-only, while assistant text streams), and `PostToolBatch` (after a parallel tool batch resolves, before the next model call).
...
**Full lifecycle events:** Codex gained a full lifecycle hook surface in early 2026, expanding from the previous notify-only design, and has kept growing. Current events (11 as of 2026-08-19): `SessionStart` and `SubagentStart` when a session or subagent starts; `SessionEnd` when the main thread ends — newly documented since the 2026-07-25 refresh, and explicitly "doesn't run for subagents"; and `PreToolUse`, `PermissionRequest`, `PostToolUse`, `UserPromptSubmit`, `SubagentStop`, `PreCompact`, `PostCompact`, and `Stop` during a turn. `SessionEnd` is observe-only (its single documented end reason is `other`) and carries a shorter timeout budget than the rest: 1 second by default, 3 seconds maximum. Configured in `hooks.json` at `~/.codex/hooks.json` (user) or `<repo>/.codex/hooks.json` (project), or inline `[hooks]` tables in `config.toml`; plugins can also bundle hooks via their manifest. Hooks are now **enabled by default** — disable with `[features] hooks = false` (`codex_hooks` survives as a deprecated alias). The older end-of-turn `notify` hook still exists, configured separately in `config.toml` under `[notify]`.
...
- **Blocking:** `PreToolUse` returns a `decision` of `allow` / `deny` / `ask` / `force_ask` / `deny_unless_prior_grant` plus `permissionOverrides` — `deny` "hard blocks execution immediately," while `deny_unless_prior_grant` denies "unless the resource was previously approved in a prior user grant."
...
- **Reliability fixes since 1.1.6.** Disabled plugins no longer run their hooks (1.1.7). `PostToolUse` stopped firing on non-tool steps such as user input and model responses — a bug that also made it ignore configured matchers — and an always-blocking `Stop` hook can no longer hang the agent forever: after a configurable number of consecutive continuations it loses the ability to block (1.1.9). Hook ordering changed so `hooks.json` hooks run _before_ the built-in termination checks, which is what finally makes `Stop` hooks reachable at all and lets `PostInvocation` observe a turn's final invocation (1.1.10).
...
The Claude Code `↔` Gemini CLI parity held from the 2026-04-11 refresh until the Gemini column retired on 2026-06-10. The 2026-07-11 refresh answered this page's biggest open question: Antigravity CLI shipped hook docs — five events with hard-blocking `PreToolUse` — restoring most, but not all, of the predecessor's surface (no input rewriting, no output substitution, no fingerprinting; all still absent as of 2026-07-25). Codex CLI keeps converging — by 2026-06-10 it had sub-agent lifecycle events, compaction events, hooks on by default, and `updatedInput` rewriting; by 2026-07-11 a hash-based hook-trust flow functionally equivalent to Gemini's fingerprinting; and by 2026-07-25 interception of the unified-exec shell and other local function tools, leaving hosted tools (`WebSearch`) as the only named hole — but the load-bearing structural difference remains: the docs reserve an opt-out for "specialized tool paths" and disclaim enforcement-boundary status, so you still can't wire a comprehensive pre-tool gate. The 2026-08-10 refresh added one event to each of the two leaders' catalogs — Claude Code's `DirectoryAdded` and Codex's `SessionEnd`, both observe-only — which moves neither verdict. Antigravity's news was corrective rather than additive: 1.1.9 and 1.1.10 fixed `PostToolUse` matchers and made `Stop` hooks reachable, meaning parts of the catalog documented on 2026-07-11 were not fully working when this page first verified them. The 2026-08-19 refresh found all three catalogs unchanged in size — 31 / 11 / 5 — with one detail added to Antigravity's: `PreToolUse` documents a fifth decision, `deny_unless_prior_grant`, which denies unless a prior user grant already covered the resource. No verdict moved.
